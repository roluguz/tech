...
?>
    <body>
        ...
        <div class="container-fluid">
            <div class="row-fluid">
            <?php // este es como esta include('notification_slidebar.php'); ?>			
    ...
    <form action="read.php" method="post">
        ...
        <?php } ?>
           <?php $query = mysqli_query($cnx, "select * from notification order by notification.date_of_notification DESC")or die(mysqli_error());
                  $count = mysqli_num_rows($query);
                            if ($count  > 0){
                            while($row = mysqli_fetch_array($query)){
                            $get_id = $row['fullname'];		                  
                            $id = $row['notification_id'];				
                            $query_yes_read = mysqli_query("select * from notification_read where notification_id = '$id' and admin_id = '$session_id'")or die(mysqli_error());
