					 ...
					 <a href="device_location.php" class="btn btn-info"  id="return" data-placement="right" title="Click to return" ><i class="icon-arrow-left icon-large"></i> Back</a>
		              ...
		              $('#return').tooltip('hide');
						...
						<?php $location_query = mysqli_query($cnx, "select * from stlocation	                     
	                     where stdev_id = '$get_id'")or die(mysqli_error());
                  ...
                  <div class="navbar navbar-inner block-header">							
							  <div class="muted pull-left"><i class="icon-building"></i>  <?php echo $location_row['stdev_location_name']; ?> </div>
                                ...
                                (select * from location_details where dev_status='Dump')
                                 and stdev_id = '$get_id' ")or die(mysqli_error());
...
<?php
 ...
 <!-- ---------------------------------device categorized by their location details using nav pills---------------------------------- -->	
	<div class="pull-left">
			...
			<?php	
	          $my_keyboard = mysqli_query($cnx, "select * from location_details    
	          LEFT JOIN stdevice ON location_details.id = stdevice.id
			  LEFT JOIN device_name ON stdevice.dev_id=device_name.dev_id
			  where NOT EXISTS 
             (select * from location_details where dev_status='Dump')
             ...
             (select * from location_details where dev_status='Dump')
			  ...
			  OR NOT EXISTS 
             (select * from location_details where dev_status='Dump')
			  and dev_name = '(kbrd)'    and stdev_id = '$get_id' ")or die(mysqli_error());
			  ...
			  ?>
				...
				<?php	
	            $my_mouse = mysqli_query($cnx, "select * from location_details    
	            LEFT JOIN stdevice ON location_details.id = stdevice.id
				LEFT JOIN device_name ON stdevice.dev_id=device_name.dev_id
				where NOT EXISTS 
                (select * from location_details where dev_status='Dump') 
			  ...
			  <?php	
	            $my_monitor = mysqli_query($cnx, "select * from location_details    
	            LEFT JOIN stdevice ON location_details.id = stdevice.id
				LEFT JOIN device_name ON stdevice.dev_id=device_name.dev_id
				where NOT EXISTS 
                (select * from location_details where dev_status='Dump')
				...
				OR NOT EXISTS 
                (select * from location_details where dev_status='Dump')
				and dev_name = '(montor)' and stdev_id = '$get_id' ")or die(mysqli_error());
               ...
               ?>
				...
				<li class="">			
				 ...
				 <?php	
	            $my_cpu = mysqli_query($cnx, "select * from location_details    
	            LEFT JOIN stdevice ON location_details.id = stdevice.id
				LEFT JOIN device_name ON stdevice.dev_id=device_name.dev_id
				where NOT EXISTS 
                (select * from location_details where dev_status='Dump') 
				...
				OR NOT EXISTS 
                (select * from location_details where dev_status='Dump')
				and dev_name = '(Central Processing unit (CPU))'  and stdev_id = '$get_id' ")or die(mysqli_error());
               ...
               <?php	
	            $my_power_supply = mysqli_query($cnx, "select * from location_details    
	            LEFT JOIN stdevice ON location_details.id = stdevice.id
				LEFT JOIN device_name ON stdevice.dev_id=device_name.dev_id
				where NOT EXISTS 
                (select * from location_details where dev_status='Dump')
				...
				OR NOT EXISTS 
                (select * from location_details where dev_status='Dump')
			    and dev_name = 'POWER SUPPLY'   and stdev_id = '$get_id' ")or die(mysqli_error());
               ...
               ?>			
				...
				<?php	
	            $my_vga = mysqli_query($cnx, "select * from location_details    
	            LEFT JOIN stdevice ON location_details.id = stdevice.id
				LEFT JOIN device_name ON stdevice.dev_id=device_name.dev_id
				where NOT EXISTS 
                (select * from location_details where dev_status='Dump')
				...
				OR NOT EXISTS 
                (select * from location_details where dev_status='Dump')
				and dev_name = 'vedio graphic accelerator'   and stdev_id = '$get_id'")or die(mysqli_error());
               ...
               ?>
				...
				OR NOT EXISTS 
                (select * from location_details where dev_status='Dump')
				and dev_name = 'power-cord'    and stdev_id = '$get_id'")or die(mysqli_error());
			    $count = mysqli_num_rows($my_vga);
               ?>				
				<li class="">			
				...
				<?php	
	            $my_avr = mysqli_query($cnx, "select * from location_details    
	            LEFT JOIN stdevice ON location_details.id = stdevice.id
				LEFT JOIN device_name ON stdevice.dev_id=device_name.dev_id
				where NOT EXISTS 
                (select * from location_details where dev_status='Dump')
...
<div class="container-fluid">
  <div class="row-fluid"> 
     <div class="empty">
        ...
        <?php
         $str = "select * from location_details	                                                    
         LEFT JOIN stdevice ON location_details.id = stdevice.id 
         LEFT JOIN device_name ON stdevice.dev_id=device_name.dev_id
         where NOT EXISTS 
         (select * from location_details where dev_status='Dump')
         ...
         //echo $str; die();
         $my_device = mysqli_query($cnx, $str) or die(mysqli_error());
         if (mysqli_num_rows($my_device)) {
...
(select * from location_details where dev_status='Dump')
and stdev_id = '$get_id'		
order by date_deployment DESC")or die(mysqli_error());
while($row = mysqli_fetch_array($my_device)){
$id = $row['id'];		                                               
?>
<tr>
		<td><?php
			   $device_query2 = mysqli_query($cnx, "select * from stdevice ")or die(mysqli_error());
			...
			<td><?php
			   $my_device1 = mysqli_query($cnx, "select * from stdevice ")or die(mysqli_error());
